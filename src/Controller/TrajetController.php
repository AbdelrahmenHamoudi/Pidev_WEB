...
final class TrajetController extends AbstractController
{
    ...
    public function create(Request $request, EntityManagerInterface $em, VoitureRepository $voitureRepo, ValidatorInterface $validator, HubInterface $hub, MailerInterface $mailer): Response
    {
        ...
        $user = $this->getUser();
        if ($user) {
            $trajet->setIdUtilisateur($user->getId());
        } else {
            $trajet->setIdUtilisateur(self::DEFAULT_USER_ID); // fallback
        }
        ...
        if ($request->isMethod('POST')) {
            ...
            if (count($errors) > 0) {
                ...
                try {
                    ...
                    $this->addFlash('warning', 'Le trajet est réservé, mais l\'email de confirmation n\'a pas pu être envoyé. Erreur : ' . $e->getMessage());
                }
